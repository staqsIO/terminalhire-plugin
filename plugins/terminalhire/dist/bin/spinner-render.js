...
import { join as join2, dirname as dirname2, basename, resolve, isAbsolute } from "path";
...
function resolveTarget(path) {
  ...
  for (let hop = 0; hop < MAX_LINK_HOPS; hop++) {
    ...
    try {
      ...
      next = isAbsolute(dest) ? dest : join2(dirname2(cur), dest);
    } catch {
      settled = true;
      break;
    }
  ...
  try {
    return join2(realpathSync(dirname2(path)), basename(path));
  } catch {
    return path;
  }
}
