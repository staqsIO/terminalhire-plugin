...
import { join as join3, dirname as dirname2, basename as basename2, resolve, isAbsolute } from "path";
...
// bin/spinner-io.js
function thDir() {
  const raw = process.env["TERMINALHIRE_DIR"] || join3(homedir3(), ".terminalhire");
  return resolve(raw);
}
function claudeSettingsPath() {
  return process.env["TERMINALHIRE_CLAUDE_SETTINGS"] || join3(homedir3(), ".claude", "settings.json");
}
function spinnerStateFilePath() {
  return join3(thDir(), "spinner-state.json");
}
...
function resolveTarget(path) {
  ...
  for (let hop = 0; hop < MAX_LINK_HOPS; hop++) {
    ...
    try {
      ...
      next = isAbsolute(dest) ? dest : join3(dirname2(cur), dest);
    } catch {
      settled = true;
      break;
    }
  ...
  try {
    return join3(realpathSync(dirname2(path)), basename2(path));
  } catch {
    return path;
  }
}
...
function writeSettingsJson(path, obj, expectedRaw) {
  ...
  let currentRaw = null;
  try {
    currentRaw = readFileSync2(target, "utf8");
  } catch (err) {
