...
function atomicWriteFileSync(filePath, content) {
  ...
  const tmp = join3(dir, `.${basename(filePath)}.tmp-${process.pid}-${randomBytes3(6).toString("hex")}`);
...
var TERMINALHIRE_DIR4, REPO_EXPERIENCE_FILE, MAX_REPOS, MAX_CULTURE_SAMPLES, MAX_NOTES, MAX_BACKFILL, store, activeStore;
var init_repo_experience = __esm({
  "src/repo-experience.ts"() {
    ...
    REPO_EXPERIENCE_FILE = join5(TERMINALHIRE_DIR4, "repo-experience.enc");
    MAX_REPOS = 100;
...
import { readFileSync as readFileSync4, writeFileSync as writeFileSync3, mkdirSync as mkdirSync3, renameSync as renameSync3, existsSync as existsSync2, rmSync as rmSync2, statSync } from "fs";
...
function withClaimsLock(fn) {
  mkdirSync3(TERMINALHIRE_DIR5, { recursive: true, mode: 448 });
...
function writeClaims(claims) {
  ...
  try {
    writeFileSync3(tmp, JSON.stringify(payload, null, 2), { encoding: "utf8", mode: 384, flag: "wx" });
...
var INDEX_CACHE_FILE2 = join7(TERMINALHIRE_DIR6, "index-cache.json");
...
async function run() {
  try {
    ...
    const { bounties, ranked, matchedCount, continuityByRepo } = result;
