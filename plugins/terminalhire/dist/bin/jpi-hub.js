...
var TERMINALHIRE_DIR5, TOKEN_FILE, ALGO2, IV_BYTES2;
...
import { existsSync as existsSync7, linkSync as linkSync2, readFileSync as readFileSync7, rmSync as rmSync4, unlinkSync as unlinkSync2, writeFileSync as writeFileSync6 } from "fs";
...
function readIdentityFileOrThrow(key) {
  try {
    const raw = readFileSync7(IDENTITY_FILE, "utf8");
    const blob = JSON.parse(raw);
...
function publishIdentityBlob(blob) {
  const tmpFile = `${IDENTITY_FILE}.${process.pid}.${randomBytes7(6).toString("hex")}.tmp`;
  try {
...
var init_chat_keystore = __esm({
  "src/chat-keystore.ts"() {
    ...
    IDENTITY_FILE = join9(TERMINALHIRE_DIR6, "chat-identity.enc");
    HEX64_RE = /^[0-9a-f]{64}$/;
  }
});
...
// src/web-session.ts
import { chmodSync, existsSync as existsSync8, readFileSync as readFileSync8, rmSync as rmSync5, writeFileSync as writeFileSync7 } from "fs";
...
function readWebSessionRecord() {
  try {
    ...
    return parseWebSessionFile(readFileSync8(path, "utf8"));
  } catch {
    return null;
  }
}
...
function defaultReadPeerPins() {
  try {
    ...
    const parsed = JSON.parse(readFileSync9(PEERS_FILE, "utf8"));
...
function defaultWritePeerPins(pins) {
  ...
  writeFileSync8(PEERS_FILE, JSON.stringify(pins, null, 2), { mode: 384, encoding: "utf8" });
}
...
var CHAT_BASE, GH_SESSION_COOKIE, TERMINALHIRE_DIR7, PEERS_FILE, REQUEST_TIMEOUT_MS, ChatNotLinkedError, ChatSessionElsewhereError, ChatSessionExpiredError, SafetyNumberChangedError, ChatRequestError;
  ...
  "src/chat-client.ts"() {
    ...
    SafetyNumberChangedError = class extends Error {
      ...
      constructor(peerLogin, pinnedKey, servedKey) {
        super(
          `\u26A0 safety number changed for @${peerLogin} \u2014 the key on file does not match the server. Verify before continuing.`
        );
...
function defaultSessionCookie() {
  return readWebSessionCookie(CHAT_BASE2);
}
function defaultSessionMismatch() {
  return webSessionCookieForHost(CHAT_BASE2).mismatch;
}
async function fetchIntroList(deps = {}) {
  ...
  if (res.status === 401) return { status: "expired", host: CHAT_BASE2 };
...
function readReadCursors() {
  try {
    ...
    const parsed = JSON.parse(readFileSync11(READS_FILE, "utf8"));
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) return {};
    const out = {};
...
var CHAT_BASE3, TERMINALHIRE_DIR8, READS_FILE, INDEX_CACHE_FILE, REACHABLE_DISPLAY;
...
var TERMINALHIRE_DIR12, REPO_EXPERIENCE_FILE, MAX_REPOS, MAX_CULTURE_SAMPLES, MAX_NOTES, MAX_BACKFILL, store, activeStore;
...
function readIndexCache() {
  try {
    const raw = readFileSync14(INDEX_CACHE_FILE3, "utf8");
...
function applyContinuityRank(bounties, continuityOf, { enabled = true } = {}) {
  ...
  const effectiveVal = (j) => {
    const s = j.winnabilityScore ?? 0;
    const repo = j.bounty?.repoFullName;
...
async function getBounties({ quiet = false, offline = false, priced = PRICED_ONLY } = {}) {
  ...
  if (!CONTINUITY_RANK_DISABLED && process.stdout.isTTY) {
    try {
      ...
      const repos = [...new Set(bounties.map((j) => j.bounty?.repoFullName).filter(Boolean))];
      for (const repo of repos) {
        continuityByRepo.set(repo, await continuityForRepo2(repo, claims));
      }
      applyContinuityRank(bounties, (repo) => continuityByRepo.get(repo)?.score ?? 0);
    } catch {
    }
  }
...
function writeDirectoryCache(index) {
  ...
  writeFileSync12(DIRECTORY_CACHE_FILE, JSON.stringify({ ts: Date.now(), index }), "utf8");
}
  ...
  return new Promise((resolve) => {
    ...
    function inboxRows(st) {
      switch (st.status) {
        ...
        case "linked-elsewhere":
          return [missingSessionLines(st).join(" ")];
        case "expired":
          return [
            st.host ? refusedSessionLines(st.host).join(" ") : "Your linked session was refused \u2014 run `terminalhire link` again."
          ];
        case "not-connected":
    ...
    function introRows(result) {
      ...
      switch (result.status) {
        ...
        case "linked-elsewhere":
          return [missingSessionLines(result).join(" ")];
        case "expired":
          return [refusedSessionLines(result.host).join(" ")];
        case "request-failed":
