...
import { readFileSync as readFileSync3, writeFileSync, existsSync as existsSync2, linkSync, unlinkSync } from "fs";
...
var init_shared_key = __esm({
  "src/shared-key.ts"() {
    ...
    TERMINALHIRE_DIR = process.env.TERMINALHIRE_DIR || join4(homedir(), ".terminalhire");
...
import { readFileSync as readFileSync4, writeFileSync as writeFileSync2, existsSync as existsSync3, renameSync, rmSync, readdirSync as readdirSync2 } from "fs";
...
async function deleteKey() {
  ...
  let encFiles;
  try {
    encFiles = readdirSync2(stateDir).filter((f) => f.endsWith(".enc"));
  } catch (e) {
...
var TERMINALHIRE_DIR2, PROFILE_FILE, profileStore, DERIVED_KEYS, LANGUAGE_TAGS2, MIN_FINGERPRINT_SCORE;
var init_profile = __esm({
  "src/profile.ts"() {
    "use strict";
    init_src();
    init_crypto_store();
    TERMINALHIRE_DIR2 = process.env.TERMINALHIRE_DIR || join6(homedir2(), ".terminalhire");
