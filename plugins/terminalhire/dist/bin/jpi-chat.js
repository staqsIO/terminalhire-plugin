...
function renderInbox(items, invites = []) {
  ...
  if (invites && invites.length > 0) {
    ...
    for (const iv of invites) {
      ...
      const handle = `@${login}`;
      lines.push(
        `  \u2198 ${handle.padEnd(18)} wants to connect \xB7 terminalhire intro --accept ${handle}`
      );
    }
    lines.push("  " + "\u2500".repeat(64));
  }
  ...
  lines.push("  " + "\u2500".repeat(64));
  lines.push(
    '  read: terminalhire chat <login> --read   \xB7   reply: terminalhire chat <login> --send "\u2026"'
  );
  return lines.join("\n") + "\n";
}
function renderThread(state) {
  ...
  const lines = [];
  lines.push(`  @${safePeer}  ${status}` + (safety ? `  \xB7 safety# ${sanitizeLine(safety)}` : ""));
  lines.push("  " + "\u2500".repeat(64));
  if (!messages || messages.length === 0) {
    ...
    for (const m of messages) {
      ...
      lines.push(
        `  ${formatClock(m.createdAt).padStart(6)}  ${who.padEnd(16)} ${sanitizeLine(m.plaintext)}`
      );
    }
  }
  lines.push("  " + "\u2500".repeat(64));
...
import { readFileSync as readFileSync8, writeFileSync as writeFileSync7, existsSync as existsSync8, renameSync as renameSync2, rmSync as rmSync4 } from "fs";
...
var init_crypto_store = __esm({
  "src/crypto-store.ts"() {
    ...
    KEY_FILE2 = join9(TERMINALHIRE_DIR6, "key");
...
var init_profile = __esm({
  "src/profile.ts"() {
    ...
    PROFILE_FILE = join10(TERMINALHIRE_DIR7, "profile.enc");
...
function readCachedSessionStale() {
  try {
    const p = join11(process.env.TERMINALHIRE_DIR || join11(homedir9(), ".terminalhire"), "index-cache.json");
    if (!existsSync9(p)) return false;
