...
__export(github_auth_exports, {
  ...
  requireStoredLogin: () => requireStoredLogin,
...
var TERMINALHIRE_DIR5, TOKEN_FILE, ALGO2, IV_BYTES2, GITHUB_SCOPE, DEVICE_CODE_URL, ACCESS_TOKEN_URL, BAKED_IN_CLIENT_ID, MOCK_TOKEN, MOCK_LOGIN;
...
var TERMINALHIRE_DIR7, CLAIMS_FILE, LOCK_DIR, LOCK_STALE_MS, LOCK_RETRY_MS, LOCK_TIMEOUT_MS, CLAIM_STATES, CLAIM_TURN_BY_STATE, CLAIM_TURN_LABEL, CLAIM_TURN_LABEL_WIDTH, PUSHED_CLAIM_FIELDS, TERMINAL_STATES, POLL_TRANSITIONS, WHITESPACE_CONTROLS2, CONTROL_CHARS2, DECIDED_STATES;
...
async function runScoped(repoFullName, { log, fetchImpl, useCache, opts }) {
  ...
  if (cookie === null && useCache) {
    try {
      const { webSessionCookieForHost: webSessionCookieForHost2 } = await Promise.resolve().then(() => (init_web_session(), web_session_exports));
      const session = webSessionCookieForHost2(API_URL);
      cookie = session.cookie;
      linkedElsewhere = session.mismatch;
    } catch {
      cookie = null;
    }
  }
  ...
  if (!listing) {
    ...
    if (!cookie) {
      sharedIndexNote = linkedElsewhere ? (await Promise.resolve().then(() => (init_web_session(), web_session_exports))).missingSessionLines(linkedElsewhere).join(" ") : "Run `terminalhire link` to add repos you look up to the shared index for everyone.";
    }
  }
