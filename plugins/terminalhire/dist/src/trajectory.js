  ...
  for (const episode of episodes) {
    ...
    for (const uuid of episode.mainNodeUuids) {
      ...
      if (node.kind === "tool_result" /* ToolResult */ && node.isError) {
        run++;
        if (run > maxConsecutiveErrors) {
          maxConsecutiveErrors = run;
        }
...
import { basename, join as join2, normalize as normalize2 } from "path";
...
function renderMarkdown(view) {
  ...
  lines.push("");
  lines.push("> Your code is your r\xE9sum\xE9. Derived from your own Claude Code corpus.");
  lines.push("");
  lines.push("## Trajectory");
  lines.push("");
  ...
  lines.push("");
  lines.push(
    `- **Live** (used this quarter): ${prettyList(view.score.liveStack.filter(isLang), 64)}`
  );
  lines.push(
    `- **Dormant** (90d+ untouched): ${prettyList(view.score.dormantStack.filter(isLang), 64)}`
  );
  lines.push("");
  ...
  lines.push(
    `- ${prettyList(
      view.score.liveStack.filter((s) => !isLang(s)),
      64
    )}`
  );
  lines.push("");
...
async function runTrajectoryPush(opts, overrides) {
  ...
  if (opts.delete) {
    const answer2 = await deps.prompt(
      '  Unlink your trajectory from terminalhire.com? Type "yes" to confirm: '
    );
    if (answer2 !== "yes") {
...
`);
  ...
  const answer = await deps.prompt(
    '  Type "yes" to link your trajectory (anything else cancels): '
  );
